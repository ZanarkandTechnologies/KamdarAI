...
export const dailyReferenceContext = resolve(projectRoot, "evals/daily/expected/reference-context.json");
export const task0007AutomationId = "kamdar-task0007-reference-automation";
export const referenceWeek = "2026-W34";
...
function safePath(root, path) {
  ...
  if (!resolved.startsWith(resolve(root) + "/")) throw new Error("TASK-0007 reference path escaped run root: " + path);
  return resolved;
}

function expectedRerunState(initial, label) {
  if (initial.state === "applied") return "duplicate";
  if (initial.state === "no_finding") return "no_finding";
  throw new Error(`Current Weekly Draft ${label} update cannot prove idempotency from initial state ${initial.state}.`);
}
function ensureRoot(root) {
  const target = resolve(root);
  if (existsSync(target) && readdirSync(target).length) throw new Error("TASK-0007 reference run root is not empty: " + target);
...
export function documentationPlan(context, contextPath) {
  ...
  return [
    ...
    "## Handoff", "", "- Delivery state: proposal-only", "- Integration owner: daily-operating-update"
  ].join("\n");
}
...
export function projectControlPlan(context, contextPath, draftUpdate, draftPath) {
  ...
  return {
    ...
    provider_effects: { performed: false, reason: "The Weekly Draft changed locally; the Daily automation owns direct sends and receipts." }
  };
}
...
function weeklyFinalization({ root, context, draftPath }) {
  ...
  validateCurrentWeeklyDraft(draftContent, { expectedWeek: referenceWeek });
  const projectPaths = context.projects.map((project) => {
    const path = "weekly/reports/projects/" + project.id + "-" + referenceWeek + ".md";
    output(root, path, [
      "---", "artifact_type: kamdar-weekly-project-report", "week: " + referenceWeek, "project_id: " + project.id, "department: Unassigned", "source_draft: " + draftPath, "---", "",
      "# " + project.name + " — " + referenceWeek, "", "## Summary", "", "Finalized from the current Weekly Draft; no raw Daily re-synthesis.", "", projectDraftContent(draftContent, project.id), "",
  ...
  const departmentPath = "weekly/reports/departments/unassigned-" + referenceWeek + ".md";
  output(root, departmentPath, [
    "---", "artifact_type: kamdar-weekly-department-report", "week: " + referenceWeek, "department: Unassigned", "---", "",
    "# Unassigned — " + referenceWeek, "", "## Source project reports", "", ...projectPaths.map((path) => "- " + path), "", "## Results and open attention", "", "See linked Project reports; Department routing is a source gap in this fixture.", "", "## Next-week handoff", "", "Resolve Project Department values before a production rollup."
  ].join("\n"));
  const companyPath = "weekly/reports/company/" + referenceWeek + ".md";
  output(root, companyPath, [
    "---", "artifact_type: kamdar-weekly-company-report", "week: " + referenceWeek, "source_draft: " + draftPath, "---", "",
    "# Company operating review — " + referenceWeek, "", "## Results and open attention", "", "Daily pipelines updated one current Weekly Draft directly before finalization.", "", "## Problems worth solving", "", "Project Department routing is missing in the fixture; do not infer it.", "", "## Decisions and SOPs awaiting review", "", draftAnchorText(draftContent, "Decisions"), "", draftAnchorText(draftContent, "SOPs"), "", "## Next-week handoff", "", "Use the finalized Project reports and preserve Proposed candidates until their review conditions are met."
  ].join("\n"));
  const keys = keysFromDraft(draftContent);
  const planPath = "weekly/finalization/weekly-finalization-plan-" + referenceWeek + ".md";
  output(root, planPath, [
    "---", "artifact_type: kamdar-weekly-finalization-plan", "artifact_version: \"0.1.0\"", "week: " + referenceWeek, "state: ready", "current_weekly_draft: " + draftPath, "---", "",
    "# Weekly finalization — " + referenceWeek, "", "## Input Draft", "", "- Path: " + draftPath, "- Source keys: " + keys.join(", "), "- Source gaps: Project Department routing is not supplied by this Daily reference input.", "", "## Report hierarchy", "", ...projectPaths.map((path) => "- Project: " + path), "- Department: " + departmentPath, "- Company: " + companyPath, "", "## Promotion review", "", ...keys.filter((key) => key.startsWith("decision:") || key.startsWith("sop:")).map((key) => "- " + key + " — retain Proposed; authority/recurrence/owner/proof require Weekly review."), "", "## Completion boundary", "", "- The input Draft was read only.", "- Provider publication and executive delivery were not invoked."
  ].join("\n"));
  const receiptPath = "weekly/receipt-" + referenceWeek + ".json";
...
export function runTask0007ReferenceAutomation({ outputRoot } = {}) {
  if (!outputRoot) throw new Error("TASK-0007 reference automation needs an explicit private output root.");
  const root = ensureRoot(outputRoot);
  const context = readJson(dailyReferenceContext);
  const contextPath = output(root, "daily/context/daily-context-diff-" + context.local_day + ".json", context);
  const draftPath = "weekly/current/weekly-report-draft-" + referenceWeek + ".md";
  initializeCurrentWeeklyDraft({ draftPath: safePath(root, draftPath), week: referenceWeek });
  ...
  const knowledgeUpdate = updateCurrentWeeklyDraft({ draftPath: safePath(root, draftPath), expectedWeek: referenceWeek, entries: dailyKnowledgeEntries(context) });
  const controlUpdate = updateCurrentWeeklyDraft({ draftPath: safePath(root, draftPath), expectedWeek: referenceWeek, entries: dailyControlEntries(context) });
  ...
  const knowledgeRerun = updateCurrentWeeklyDraft({ draftPath: safePath(root, draftPath), expectedWeek: referenceWeek, entries: dailyKnowledgeEntries(context) });
  const controlRerun = updateCurrentWeeklyDraft({ draftPath: safePath(root, draftPath), expectedWeek: referenceWeek, entries: dailyControlEntries(context) });
  const rerunAfter = sha256(readFileSync(safePath(root, draftPath), "utf8"));
  const expectedKnowledgeRerun = expectedRerunState(knowledgeUpdate, "knowledge");
  const expectedControlRerun = expectedRerunState(controlUpdate, "control");
  if (knowledgeRerun.state !== expectedKnowledgeRerun || controlRerun.state !== expectedControlRerun || rerunBefore !== rerunAfter) {
    throw new Error(`Current Weekly Draft rerun was not a zero-write duplicate: knowledge=${knowledgeRerun.state}, control=${controlRerun.state}, hash_unchanged=${rerunBefore === rerunAfter}.`);
  }
  ...
  const result = {
    schema_version: 2,
    kind: "task0007-reference-automation-run",
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else throw new Error("usage: node run-task0007-reference-automation.mjs --output <new-private-run-directory>");
  }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try { process.stdout.write(stable(runTask0007ReferenceAutomation(parseArgs(process.argv.slice(2))))); }
