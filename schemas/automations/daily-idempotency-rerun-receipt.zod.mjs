...
const AuditEffectSchema = z.strictObject({
  ...
  result_pointer: z.string().regex(/^\/(project_updates|documentation_reviews|weekly_progress_chases|knowledge_updates)(?:\/\d+)?$/),
...
const WorkProcessingAuditSchema = z.strictObject({
  ...
  original_state: z.enum(["processed", "needs_information", "blocked"]),
  rerun_state: z.enum(["processed", "needs_information", "blocked"]),
  ...
  const fieldsMatch =
    (row.rerun_state === "processed" && row.ai_review_after === "Processed" && row.daily_review_version_after === "daily-review-v2")
    || (row.rerun_state === "needs_information" && row.ai_review_after === "Needs information" && row.daily_review_version_after === null)
    || (row.rerun_state === "blocked" && row.ai_review_after === "Blocked" && row.daily_review_version_after === null);
  if (!fieldsMatch) context.addIssue({ code: "custom", message: "AI review properties must match rerun_state." });
});

export const DailyIdempotencyRerunReceiptSchema = z.strictObject({
  schema_version: z.literal("kamdar-daily-idempotency-rerun-receipt@1.1.0"),
