...
// stops at daily-review-result.zod.mjs; provider application emits this receipt
...
const IntegrationEffectSchema = z
  .object({
    ...
    result_pointer: z
      .string()
      .regex(/^\/(project_updates|documentation_reviews|weekly_progress_chases|knowledge_updates)(?:\/\d+(?:\/.*)?)?$/)
...
const WorkProcessingDecisionSchema = z
  .object({
    ...
    documentation_review_pointer: z.string().regex(/^\/documentation_reviews\/\d+$/),
    documentation_verdict: z.enum(["sufficient", "needs_information"]),
    required_effect_ids: z.array(StableIdSchema).min(1),
    state: z.enum(["processed", "needs_information", "blocked"]),
    processed_at: z.string().datetime({ offset: true }).nullable(),
    status_after: z.literal("Done"),
    ai_review_after: z.enum(["Needs information", "Processed", "Blocked"]),
    daily_review_version_after: z.literal("daily-review-v2").nullable(),
...
function processingSafetyErrors(receipt) {
  ...
  for (const decision of receipt.work_processing ?? []) {
    ...
    if ((decision.state === "processed") !== canProcess) errors.push(`${decision.work_item_id} may be processed only when documentation is sufficient and every required effect safely settles`);
    if ((decision.state === "needs_information") !== needsInformation) errors.push(`${decision.work_item_id} must remain Needs information after its documentation question safely settles`);
    if ((decision.state === "blocked") !== !safelySettled) errors.push(`${decision.work_item_id} must be Blocked while a required effect is blocked, conflicted, failed, or missing`);
    ...
    const stateFieldsMatch =
      (decision.state === "processed" && decision.ai_review_after === "Processed" && decision.daily_review_version_after === "daily-review-v2")
      || (decision.state === "needs_information" && decision.ai_review_after === "Needs information" && decision.daily_review_version_after === null)
      || (decision.state === "blocked" && decision.ai_review_after === "Blocked" && decision.daily_review_version_after === null);
    if (!stateFieldsMatch) errors.push(`${decision.work_item_id} has AI review fields that do not match its processing state`);
  }
...
export const DailyIntegrationReceiptSchema = z
  .object({
    schema_version: z.literal("kamdar-daily-integration-receipt@1.1.0"),
