...
// documentation_reviews[] with verdict=needs_information
//   -> notion via ntn (one deduplicated rendered Work comment per row)
// weekly_progress_chases[]
//   -> notion Person lookup, then the approved configured channel
...
//
// Application still requires guarded section/comment writes and provider
// receipts; extraction must not claim those effects.
...
/**
 * FEAT-0002 — Completed-ticket documentation review
 *
 * Feature:
 ...
 *
 * Operation:
 * Return one verdict for every Done Work item selected for review. A
 * needs_information verdict carries one precise source-record comment; a
 * sufficient verdict carries no question.
 */
...
export const DocumentationReviewSchema = z
  .object({
    ...
    source_ids: SourceIdsSchema,
    rubric_id: z.literal("task-completion@1.0.0"),
    verdict: z.enum(["sufficient", "needs_information"]),
    missing_requirement_ids: z.array(StableIdSchema),
    question_key: StableIdSchema.nullable().describe("Stable deduplication key for the open documentation question."),
    comment_text: z.string().min(1).nullable().describe("The complete comment to add, or null when documentation is sufficient."),
  })
  .strict()
  .superRefine((review, context) => {
    const needsInformation = review.verdict === "needs_information";
    if (needsInformation && review.missing_requirement_ids.length === 0) {
      context.addIssue({ code: "custom", message: "needs_information requires at least one missing requirement." });
    }
    if (needsInformation !== Boolean(review.question_key && review.comment_text)) {
      context.addIssue({ code: "custom", message: "needs_information requires a question key and comment; sufficient forbids both." });
    }
    if (!needsInformation && review.missing_requirement_ids.length > 0) {
      context.addIssue({ code: "custom", message: "sufficient documentation cannot list missing requirements." });
    }
  })
