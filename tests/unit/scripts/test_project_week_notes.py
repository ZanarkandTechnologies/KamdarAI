...
class ProjectWeekNotesTests(unittest.TestCase):
    ...
    def test_employee_identifier_cannot_escape_private_memory_root(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            week_root = Path(temporary) / "weeks/2026-W36"
            path = week_root / "project-notes/project--PROJ-A.md"
            initialize_project_week_notes(
            ...
            with self.assertRaisesRegex(ValueError, "employee_id is missing or unsafe"):
                append_project_week_notes(
                    notes_path=path,
                    expected_week="2026-W36",
                    expected_project_id="PROJ-A",
                    notes=[note("PROJ-A", employee_ids=["../../secrets/employee"])],
                )
